$(function() {
  ...
  const inputHandler = function(e) {
    if(e.target.value==""){
      ...
      $('.home-button').attr('disabled', false);
    }
  }
  source.addEventListener('input', inputHandler);
    $('.home-button').on('click',function(){
        var my_api_key = '0e11fe79ab7f244860ee82fc2eaf478a';
        var title = "Toy story";
        //  $('.movie').val();
        if (title=="") {
          ...
          hload_details(my_api_key,title);
        }
  });
...
function hload_details(my_api_key, title) {
    $.ajax({
        type: 'GET',
        ...
        success: function (movie) {
            if (movie.results.length < 1) {
                ...
                hmovie_recs(movie_title, movie_id, my_api_key);
            }
        },
        error: function () {
...
function hmovie_recs(movie_title, movie_id, my_api_key) {
    $.ajax({
        type: 'POST',
        url: "/similarity",
        ...
        success: function (recs) {
            if (recs == "Sorry! The movie you requested is not in our database. Please check the spelling or try with some other movies") {
                ...
                hget_movie_details(movie_id, my_api_key, arr, movie_title);
            }
        },
        error: function () {
            alert("error recs");
...
function hget_movie_details(movie_id, my_api_key, arr, movie_title) {
    $.ajax({
        type: 'GET',
        url: 'https://api.themoviedb.org/3/movie/' + movie_id + '?api_key=' + my_api_key,
        success: function (movie_details) {
            hshow_details(movie_details, arr, movie_title, my_api_key, movie_id);
        },
        error: function () {
            alert("API Error!");
            $("#loader").delay(500).fadeOut();
...
function hshow_details(movie_details, arr, movie_title, my_api_key, movie_id) {
   ...
   // var vote_count = movie_details.vote_count;
    ...
    arr_poster = hget_movie_posters(arr, my_api_key);

    // movie_cast = hget_movie_cast(movie_id, my_api_key);

    // ind_cast = hget_individual_cast(movie_cast, my_api_key);
...
// function hget_individual_cast(movie_cast, my_api_key) {
//     cast_bdays = [];
//     cast_bios = [];
//     cast_places = [];
...
// function hget_movie_cast(movie_id, my_api_key) {
//     cast_ids = [];
//     cast_names = [];
//     cast_chars = [];
//     cast_profiles = [];
...
function hget_movie_posters(arr, my_api_key) {
    var arr_poster_list = []
    for (var m in arr) {
        $.ajax({
            type: 'GET',
